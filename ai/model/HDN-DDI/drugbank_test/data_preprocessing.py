...
PKL_FILE = os.path.join(_BASE_DIR, "DrugBank", "id_data_dict_dsn_full_connect.pkl")
SMILES_FILE = os.path.join(_BASE_DIR, "DrugBank", "drug_smiles.csv")
DDI_FILE = os.path.join(_BASE_DIR, "DrugBank", "ddis.csv")

with open(PKL_FILE, "rb") as f:
    id_data_dict = pkl.load(f)

df_drugs_smiles = pd.read_csv(SMILES_FILE)

DRUG_TO_INDX_DICT = {
    drug_id: indx for indx, drug_id in enumerate(df_drugs_smiles["drug_id"])
}

drug_id_mol_graph_tup = [
    (id, Chem.MolFromSmiles(smiles.strip()))
    for id, smiles in zip(df_drugs_smiles["drug_id"], df_drugs_smiles["smiles"])
]
...
AVAILABLE_ATOM_SYMBOLS = list(
    {
        ...
        for a in itertools.chain.from_iterable(
            m[1].GetAtoms() for m in drug_id_mol_graph_tup
        )
    }
)
    ...
    if mode == "one_hot":
        atom_feature = torch.cat(
            [
                ...
                one_of_k_encoding_unk(
                    atom.GetImplicitValence(), AVAILABLE_ATOM_VALENCE
                ),
...
def get_mol_edge_list_and_feat_mtx(mol_graph):
    ...
    edge_list = torch.LongTensor(
        [(b.GetBeginAtomIdx(), b.GetEndAtomIdx()) for b in mol_graph.GetBonds()]
    )
    if len(edge_list):
        undirected_edge_list = torch.cat(
            [edge_list, edge_list[:, [1, 0]]], dim=0
        ).mT.contiguous()
    else:
...
MOL_EDGE_LIST_FEAT_MTX = {
    drug_id: get_mol_edge_list_and_feat_mtx(mol)
    for drug_id, mol in drug_id_mol_graph_tup
}

MOL_EDGE_LIST_FEAT_MTX = {
    drug_id: mol for drug_id, mol in MOL_EDGE_LIST_FEAT_MTX.items() if mol is not None
}
...
df_all_pos_ddi = pd.read_csv(DDI_FILE)
all_pos_tup = [
    (h, t, r)
    for h, t, r in zip(
        df_all_pos_ddi["d1"], df_all_pos_ddi["d2"], df_all_pos_ddi["type"]
    )
]
...
class DrugDataset(Dataset):
    def __init__(
        self, tri_list, ratio=1.0, neg_ent=1, disjoint_split=True, shuffle=True
    ):
        ...
        self.drug_ids = np.array(
            [id for id in self.drug_ids if id in MOL_EDGE_LIST_FEAT_MTX]
        )
    ...
    def collate_fn(self, batch):
        ...
        for h, t, r in batch:
            ...
            pos_b_graph = self._create_b_graph(
                get_bipartite_graph(h_data, t_data), h_data.x, t_data.x
            )
        ...
        for i in range(len(neg_h_test)):
            ...
            neg_b_graph = self._create_b_graph(
                get_bipartite_graph(neg_h_samples[i], neg_t_samples[i]),
                neg_h_samples[i].x,
                neg_t_samples[i].x,
            )
    ...
    def __corrupt_ent(self, other_ent, r, other_ent_with_r_dict, max_num=1):
        ...
        while current_size < max_num:
            ...
            mask = np.isin(
                candidates,
                other_ent_with_r_dict[(other_ent, r)],
                assume_unique=True,
                invert=True,
            )
    ...
    def __normal_batch(self, h, t, r, neg_size):
        ...
        return (
            self.__corrupt_head(t, r, neg_size_h),
