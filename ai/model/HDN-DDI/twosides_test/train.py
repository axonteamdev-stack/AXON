...
# from data_preprocessing_BDN import DrugDataset, DrugDataLoader
import warnings
...
parser.add_argument(
    "--n_atom_feats", type=int, default=66, help="num of input features"
)
parser.add_argument(
    "--n_atom_hid", type=int, default=128, help="num of hidden features"
)
parser.add_argument(
    "--rel_total", type=int, default=963, help="num of interaction types"
)
parser.add_argument("--lr", type=float, default=1e-3, help="learning rate")
parser.add_argument("--n_epochs", type=int, default=200, help="num of epochs")
parser.add_argument(
    "--kge_dim", type=int, default=128, help="dimension of interaction matrix"
)
...
parser.add_argument(
    "--pkl_name", type=str, default=f'./pkl/ts-{time.strftime("%m%d_%H%M")}.pkl'
)
...
device = f"cuda" if torch.cuda.is_available() and args.use_cuda else "cpu"
print(args)
############################################################
...
def split_train_valid(data, fold, val_ratio=0.2):
    ...
    cv_split = StratifiedShuffleSplit(
        n_splits=1, test_size=val_ratio, random_state=fold
    )
...
df_ddi_train = pd.read_csv(
    os.path.join(_BASE_DIR, "Twosides", f"fold{args.fold}", "train.csv")
)
df_ddi_test = pd.read_csv(
    os.path.join(_BASE_DIR, "Twosides", f"fold{args.fold}", "test.csv")
)
train_tup = [
    (h, t, r, n)
    for h, t, r, n in zip(
...
def train(
    ...
    for i in range(1, n_epochs + 1):
        ...
        for batch in train_data_loader:
            ...
            model.train()
            p_score, n_score, probas_pred, ground_truth = do_compute(
                batch, device, model
            )
        ...
        print(
            f"Epoch: {i}{flag} ({time.time() - start:.4f}s), train_loss: {train_loss:.4f}, val_loss: {val_loss:.4f}"
        )
        print(
            f"\t\ttrain_acc: {train_acc:.4f}, train_roc: {train_auc_roc:.4f},train_aupr: {train_ap:.4f},train_f1:{train_f1:.4f}"
        )
        print(
            f"\t\tval_acc: {val_acc:.4f}, val_roc: {val_auc_roc:.4f}, val_aupr:{val_ap:.4f}, val_f1:{val_f1:.4f}"
        )
...
def test(test_data_loader, model):
    ...
    print("============================== Test Result ==============================")
    print(
        f"\t\ttest_acc: {test_acc:.4f}, test_roc: {test_auc_roc:.4f}, test_f1: {test_f1:.4f}, test_precision: {test_precision:.4f},test_recall: {test_recall:.4f},test_int_ap: {test_int_ap:.4f},test_ap: {test_ap:.4f}"
    )
...
test_model.load_state_dict(torch.load(pkl_name, map_location=device))
