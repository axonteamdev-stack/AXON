  ...
  Widget build(BuildContext context) {
    return BlocBuilder<IntakeTimeCubit, IntakeTimeState>(
      builder: (context, state) {
        final intakeCubit = context.read<IntakeTimeCubit>();
        ...
        return Container(
          ...
          child: Row(
            ...
            children: [
              GestureDetector(
                onTap: () =>
                    _pickTime(context, intakeCubit, medicineCubit, state),
                child: Column(
                  children: [
                    TextApp(
  ...
  Future<void> _pickTime(
    ...
    final picked = await showTimePicker(
      ...
      initialTime: TimeOfDay(
        hour: state.isAm ? state.hour % 12 : (state.hour % 12) + 12,
        minute: state.minute,
      ),
    );
    if (picked != null) {
      final isAm = picked.hour < 12;
      // للعرض في الواجهة (12 ساعة)
      final hour12 = picked.hour % 12 == 0 ? 12 : picked.hour % 12;
      intakeCubit.setPickedTime(
        hour: hour12,
        minute: picked.minute,
        isAm: isAm,
      ); // للإرسال إلى الـ API (24 ساعة بدون AM/PM)
      final formattedTime =
          "${picked.hour.toString().padLeft(2, '0')}:${picked.minute.toString().padLeft(2, '0')}";
      medicineCubit.setIntakeTimes([formattedTime]);
    }
  }
