...
export const getMyMedications = catchAsync(async (req, res, next) => {
  ...
  sendResponse(res, 200, "MEDICATIONS_LIST_FETCHED_SUCCESS", medications);
});
...
export const addMedication = catchAsync(async (req, res, next) => {
  ...
  if (start < startOfToday) {
    return next(new AppError("START_DATE_IN_PAST", 400));
  }
  if (end < now) {
    return next(new AppError("END_DATE_IN_PAST", 400));
  }
  ...
  if (existingMed) {
    return next(new AppError("MEDICATION_ALREADY_ACTIVE", 400));
  }
  ...
  sendResponse(res, 201, "MEDICATION_ADDED_SUCCESS", medication);
});
...
export const updateMedication = catchAsync(async (req, res, next) => {
  ...
  if (req.body.startDate || req.body.endDate) {
    ...
    if (end < start) {
      return next(new AppError("END_DATE_INVALID", 400));
    }
  ...
  sendResponse(res, 200, "MEDICATION_UPDATED_SUCCESS", updatedMed);
});
...
export const getSingleMedication = catchAsync(async (req, res, next) => {
  ...
  if (!medication) {
    return next(new AppError("MEDICATION_NOT_FOUND", 404));
  }

  sendResponse(res, 200, "MEDICATION_FETCHED_SUCCESS", medication);
});
...
export const deleteMedication = catchAsync(async (req, res, next) => {
  ...
  sendResponse(res, 200, "MEDICATION_DELETED_SUCCESS");
});
