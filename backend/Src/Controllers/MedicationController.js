...
export const markAsTaken = catchAsync(async (req, res, next) => {
    ...
    if (!nextDose) {
        return next(new AppError({ ar: 'لقد أكملت جميع جرعات اليوم بالفعل', en: 'All doses for today completed' }, 400));
    }
    ...
    sendResponse(res, 200, { ar: "تم تسجيل أخذ الجرعة بنجاح", en: "Dose marked as taken" }, medication);
});

// 5. تسجيل الجرعة كـ "تخطي" (Skip)
export const skipDose = catchAsync(async (req, res, next) => {
    ...
    const nextDose = medication.dailyTracker.find(d => d.status === 'pending');
    if (!nextDose) {
        return next(new AppError({ ar: 'لا توجد جرعات متبقية لتخطيها اليوم', en: 'No doses left to skip today' }, 400));
    }
    ...
    sendResponse(res, 200, { ar: "تم تخطي الجرعة بنجاح", en: "Dose skipped successfully" }, medication);
});
