...
export const updateUser = catchAsync(async (req, res, next) => {
    ...
    if (req.body?.specialization) updateData['doctorProfile.specialization'] = req.body.specialization;
    if (req.body?.yearsExperience) updateData['doctorProfile.yearsExperience'] = req.body.yearsExperience;
    if (req.body?.medicalLicenseNumber) updateData['doctorProfile.medicalLicenseNumber'] = req.body.medicalLicenseNumber;

    // 4. تحديث بيانات المريض
    if (req.body?.bloodType) updateData['medicalProfile.bloodType'] = req.body.bloodType;
    if (req.body?.weight) updateData['medicalProfile.weight'] = req.body.weight;

    // 5. معالجة الصور من req.files
    if (req.files && req.files.length > 0) {
        req.files.forEach(file => {
            if (file.fieldname === 'personalPhoto') updateData.personalPhoto = file.path;
            if (file.fieldname === 'licenseImage') updateData['doctorProfile.licenseImage'] = file.path;
            if (file.fieldname === 'radiologyImage') updateData['medicalProfile.radiologyImage'] = file.path;
        });
    }
...
export const deleteUser = catchAsync(async (req, res, next) => {
    ...
    if (!user) return next(new AppError('المستخدم غير موجود أو تم حذفه بالفعل', 404));
